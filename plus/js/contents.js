$(document).ready(function(){
    ...
    /*
        ...
        영역의 하단이 브라우저 하단 위로 올라오면 : 인터랙티브 종료
    ...
    */
    ...
    function ceo_ani(){
        ...
        let ceo_area_start = ceo_area_name.offset().top //시작위치 (맨 위에서부터 계산함)
        let ceo_area_end = ceo_area_start + ceo_area_name.height() - ceo_win_h //끝 위치
        let ceo_total = ceo_area_end - ceo_area_start //전체 스크롤값
        let ceo_diff  //진행중 이후에 스크롤된값
        let ceo_per  //스크롤된값이 몇%인지

        // console.log(ceo_total)

        /*
            인터랙티브 진행중일때 몇%스크롤 했는지 계산 해야함
            ( ex : 1000px 동안 인터랙티브 할건데 100px 스크롤함 = 10% )
        ...
        */
        ...
        if(ceo_scroll > ceo_area_end){
            ...
            ceo_obj_count = ceo_obj_count * 1.3
            ...
            ceo_obj_count = ceo_obj_count + 1.3
            if(ceo_obj_count > ceo_obj_bg_end){
    ...
    if(ceo_lenght > 0){
        ceo_ani() //브라우저가 로딩되었을때 단한번 실행
    }
    $(window).scroll(function(){
        if(ceo_lenght > 0){
            ceo_ani() //브라우저가 스크롤 될때마다 실행
        }
    })
   ...
   const snbScroll = function() {
    ...
    /*
        .ctn_history .his_head가 여러개 (4개임)
        4개 각각의 애니메이션 시작시기와 종료시기를 계산해야함
    ...
    */
    ...
    function his_head(){
       ...
       for(i=0; i<obj_length; i++){
            ...
            // console.log(i, '번째', scrolling, obj_start)
    ...
    function his_area(){
        //함수 안에서 선언한 변수명은 지역변수임 
        //(다른 함수 안에 있는 변수와 변수명이 같아도됨 - 이 함수에서만 적용되는 이름)
        ...
        for(i=0; i<obj_length; i++){
            obj_top = obj_name.eq(i).offset().top
            obj_start = obj_top - win_h + (win_h * 0.3)
            obj_end = obj_top + obj_name.eq(i).height() - (win_h * 0.7)
    ...
    $(window).resize(function(){ //브라우저가 리사이즈 할때마다 실행
        if(history_length > 0){
            his_head()
            his_area()
            his_nav()
